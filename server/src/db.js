...
export async function getCompanyAccounts(id) { if(!pool)return[];const r=await pool.query(`SELECT id,label,institution,document,bank_code,agency,account,aliases,active FROM company_accounts WHERE company_id=$1 AND active=true ORDER BY created_at`,[id]);return r.rows }
export async function getChartAccounts(companyId,{includeInactive=false,includeGroups=true}={}) { if(!pool)return[];const where=[`company_id=$1`];if(!includeInactive)where.push(`active=true`);if(!includeGroups)where.push(`is_group=false`);const r=await pool.query(`SELECT id,code,name,parent_id,account_type,dre_section,dre_order,is_group,active FROM chart_accounts WHERE ${where.join(' AND ')} ORDER BY dre_order,code NULLS LAST,name`,[companyId]);return r.rows }
export async function findAccountByName(companyId,name){if(!pool||!companyId||!name)return null;const r=await pool.query(`SELECT id,code,name,parent_id,account_type,dre_section,dre_order,is_group,active FROM chart_accounts WHERE company_id=$1 AND active=true AND is_group=false AND upper(trim(name))=upper(trim($2)) LIMIT 1`,[companyId,name]);return r.rows[0]||null}

export async function audit(companyId, action, entityType=null, entityId=null, details={}, actor='MASTER') {
  if(!pool||!companyId)return
  await pool.query(`INSERT INTO audit_log(company_id,action,entity_type,entity_id,details,actor) VALUES($1,$2,$3,$4,$5::jsonb,$6)`,[companyId,action,entityType,entityId?String(entityId):null,JSON.stringify(details||{}),actor])
}
