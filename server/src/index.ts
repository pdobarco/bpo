...
app.get('/api/health',async(req,res)=>{if(!pool)return res.json({ok:true,version:'0.4.0',database:'not_configured'});try{const r=await pool.query(`SELECT value FROM schema_meta WHERE key='schema_version' LIMIT 1`);res.json({ok:true,version:'0.4.0',database:'ok',schema:r.rows[0]?.value||'unknown'})}catch(e){res.status(503).json({ok:false,version:'0.4.0',database:'migration_failed',message:e.message})}})
...
async function start(){
  ...
  console.log(`Clara BPO v0.4.0 on :${PORT}`)
}
